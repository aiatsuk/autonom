...
def remove_pins(udid: str, pins: dict[str, dict[str, Any]]) -> dict[str, list[str]]:
    """Delete the owned keys so the system defaults apply again."""
    require_preferences_dir(udid)
    removed: dict[str, list[str]] = {}
    for domain, keys in owned_keys(pins).items():
        current = read_domain(udid, domain)
        gone = [key for key in keys if key in current]
        if not gone:
            continue
        for key in gone:
            del current[key]
